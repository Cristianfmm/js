    ...
    $("#bodyPage").load("views/login.html",function(){
        $(function(){
            $("#form_login_corp").submit(function(e){
                ...
                logincorp(usuario,pass).done(function(response){
                    console.log(response);
                    if(response['code'] == 200 && response["response"]["data"] != false && response["response"]["perfil"] == 100){
                        viewPrincipal();
             ...
             /*        if(response['code'] == 200 && response["response"]["data"] != false && (response["response"]["perfil"] == 0 || response["response"]["perfil"] == 55 || response["response"]["perfil"] == 10 || response["response"]["perfil"] == 5) || response["response"]["perfil"] == 16){
                        
                                    viewPrincipal();
                                    console.log("Entro a pefil 0");
                                
                    }
                    else if(response['code'] == 200 && response["response"]["data"] != false && response["response"]["perfil"] != 0){
                        console.log(response["response"]["path"]);
                        console.log("Entro a pefil Diferente 0");
                        location.href = response["response"]["path"];
                    }
                    else if (response['code'] == 200 && response["response"]["data"] == false) {
                        message = {"400":"Intente de Nuevo", "service":"Usuario y/o contraseña incorrectos."}
